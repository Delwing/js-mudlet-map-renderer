...
import Area, { AreaBounds, Room } from "../reader/Area";
...
export class Settings {
    ...
    fontFamily: string;
    mapBackground: paper.Color;
    linesColor: paper.Color;
...
export default class Renderer {
    ...
    isVisual: boolean;
    emitter: EventTarget;
    element: HTMLCanvasElement | paper.SizeLike;
    backgroundLayer: paper.Layer;
    bgLabels: paper.Layer;
    linkLayer: paper.Layer;
    roomLayer: paper.Layer;
    rasterLayer: paper.Layer;
    labelsLayer: paper.Layer;
    specialLinkLayer: paper.Layer;
    ...
    pngRender: boolean;
    matrix: paper.Matrix;
    linkRaster: paper.Raster;
    roomRaster: paper.Raster;
    controls: Controls;
    position: paper.Item;
    selection: paper.Item;

    constructor(element: HTMLCanvasElement | paper.SizeLike | undefined, reader: MapReader, area: Area, colors: Record<string, number[]>, settings: Settings) {
    ...
    renderBackground(x1, y1, x2, y2) {
        ...
        background.registerClick(() => this.emitter.dispatchEvent(new CustomEvent("backgroundClick")));
    }

    renderHeader(x: number, y: number) {
        if (this.settings.areaName) {
            this.backgroundLayer.activate();
            let header = new paper.PointText(new paper.Point(x, y));
    ...
    renderLink(room: Room, targetId: number, dir: string) {
        ...
        let path: paper.Path | paper.Group = new paper.Path();
    ...
    renderSpecialLink(room: Room, targetId: number, dir: string) {
        this.linkLayer.activate();

        let path: paper.Path | paper.Group;
    ...
    renderChar(room) {
        ...
        if (room.roomChar) {
            ...
            let text = new paper.PointText(new paper.Point(x, room.render.position.y + size / 4));
            if (!room.userData || room.userData["system.fallback_symbol_color"] === undefined) {
                text.fillColor = new paper.Color(this.lightnessDependantColor(room));
            } else {
    ...
    renderDoors(firstPoint: paper.Point, secondPoint: paper.Point, type) {
        this.specialLinkLayer.activate();
        let x = (firstPoint.x + secondPoint.x) / 2;
        let y = (firstPoint.y + secondPoint.y) / 2;
        let door = new paper.Path.Rectangle(new paper.Point(x - 0.5, y - 0.5), new paper.Size(1, 1));
    ...
    renderLabel(value) {
        if (value.pixMap) {
            ...
            let text = new paper.PointText(background.bounds.center.add(new paper.Point(0, 0.15 * 8)));
    ...
    renderPosition(id: number, color?: number[]) {
        this.clearPosition();
        this.overlayLayer.activate();
    ...
    renderSelection(id: number, color?: number[]) {
        this.clearSelection();
        this.overlayLayer.activate();
        let room = this.area.getRoomById(id);
    ...
    renderHighlight(id: number, color: number[]) {
        this.overlayLayer.activate();
        let room = this.area.getRoomById(id);
    ...
    renderPath(locations: number[], color?: number[]) {
        this.overlayLayer.activate();
        let group = new paper.Group();
    ...
    exportSvg(roomId, padding) {
        let bounds: string | paper.Rectangle = "content";
        if (roomId !== undefined) {
            let room = this.reader.roomIndex[roomId];
            if (room === undefined) {
                throw new Error(`Room ${roomId} not found.`);
            }
            bounds = new paper.Rectangle(this.getRealPoint(room.x, room.y).subtract(padding * this.scale), new paper.Size(padding * 2 * this.scale, padding * 2 * this.scale));
        }
