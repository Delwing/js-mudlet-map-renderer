import PathFinder from "../reader/PathFinder";
import paper from "paper";
import { Room } from "../reader/Area";
import Renderer from "./Renderer";
import MapReader from "../reader/MapReader";

let selectionStyle = function(item: paper.Item): paper.Style {
    let style = new paper.Style({
    ...
    if (item.closed) {
        style.fillColor = {
            // @ts-ignore
            gradient: {
                stops: [new paper.GradientStop(item.fillColor, 0.38), new paper.GradientStop(new paper.Color(1, 1, 1))],
                radial: false
            },
...
export default class Controls {
    ...
    constructor(renderer: Renderer, reader: MapReader, element: HTMLCanvasElement, paperScope: paper.PaperScope) {
        ...
        this.activateDrag();
        this.renderer.emitter.addEventListener("roomClick", (event: CustomEvent) => this.selectRoom(event.detail));
        this.renderer.emitter.addEventListener("backgroundClick", () => this.deselectRoom());
        this.renderer.emitter.addEventListener("areaArrowClick", (event: CustomEvent) => this.goToRoomArea(event.detail));
    ...
    centerOnItem(item: paper.Item) {
        this.view.center = item.localToGlobal(item.position);
    }

    goToRoomArea(id: number) {
    ...
    renderPath(from: number, to: number, color) {
